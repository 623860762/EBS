...
<html>
  ...
  <div class="link_button_div">
    <table id="eptexperbaseinfo"></table>
    
</div>
    
    <div id="eptexperbaseinfomm" class="easyui-menu" style="width:120px;">  
...
<%--		<div iconCls="icon-tip" id="forVerify">提交审核(工作流)</div> --%>
	...
	var eptexperbaseinfo;
	var currentIndex;
	$(window).ready(function(){
		
		$('#eptexperbaseinfomm').menu({   
	...
	$(function(){
		$('#eptexperbaseinfo').datagrid({   
			...
			onLoadSuccess:function(){
				eptexperbaseinfo = $('#eptexperbaseinfo').data('datagrid').data;
			},
		    columns:[[
		    ...
		    onClickRow: function(rowIndex, rowData){
				$('#eptexperbaseinfo').datagrid('selectRow', rowIndex);
				//var row = $('#eptexperbaseinfo').dataGrid('getSelected');
				currentIndex = rowIndex;
				$('#eptexperbaseinfomm').menu('show', {
					left:event.clientX,
					top:event.clientY
				});
			},
			...
			onRowContextMenu: function(e, rowIndex, rowData) { //右键时触发事件
                ...
                $('#eptexperbaseinfo').datagrid("clearSelections"); //取消所有选中项
                $('#eptexperbaseinfo').datagrid("selectRow", rowIndex); //根据索引选中该行
                currentIndex = rowIndex;
                $('#eptexperbaseinfomm').menu('show', {
//显示右键菜单
	...
	function forMerge(){
		//alert("adb");
		var projectId = eptexperbaseinfo.rows[currentIndex].projectId;
		//var projectId = 2;
		//alert(projectId);
